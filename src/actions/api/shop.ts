import { API_BASE_URL, DEFAULT_HEADERS, handleResponse } from ".";
...
export async function getShopByName(name: string): Promise<ApiResponse<Shop>> {
  const response = await fetch(`${API_BASE_URL}/shop/name`, {
    method: "POST",
    headers: DEFAULT_HEADERS,
...
export async function getCart(
  ...
  const response = await fetch(
    `${API_BASE_URL}/cart/find?walletAddress=${walletAddress}`,
    {
      method: "GET",
      headers: DEFAULT_HEADERS,
    }
...
export async function getStoreProducts(
  ...
  const response = await fetch(`${API_BASE_URL}/store/${id}/products`, {
    method: "GET",
    headers: DEFAULT_HEADERS,
  });
  return handleResponse<Product[]>(response);
